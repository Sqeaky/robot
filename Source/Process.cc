...
// Copied with written permission from Stackover flow per creative commons licence by attribution
...
extern "C"
{
        ...
        // Windows vista check but only works with complete headers. A lack of
        // complete headers is exactly what this works around. So in the
        // absence of a better answer here is remarked code.
...
ProcessList Process::GetList (const char* name)
{
    ...
    while ((entry = readdir (dir)) != nullptr)
    {
        ...
        if (process.Open (pid))
        {
            if(empty == true || regex_match (process.mData->Name, regexp))
            {
